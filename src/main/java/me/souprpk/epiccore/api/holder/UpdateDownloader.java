package me.souprpk.epiccore.api.holder;

import me.souprpk.epiccore.EpicCore;
...
public class UpdateDownloader {
    ...
    public void checkAutoDownload(JavaPlugin plugin, int resourceId) {
        Updater updater = new Updater(plugin, resourceId,
                false); // EpicCore.getInstance().getOptions().isAutoDownload()
        ...
        try {
            download(
                    ...
                    new File(Bukkit.getServer().getUpdateFolderFile(),
                            EpicCore.getInstance().getDescription().getName() + ".jar"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        // String site =
